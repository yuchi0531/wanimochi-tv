use crate::{ts::TS_PACKET_SIZE, Error};
...
impl UsbTransport {
    ...
    fn reg_value(reg: u32) -> (u16, u16) {
        (((reg >> 8) & 0xf00) as u16, (reg & 0xffff) as u16)
    }
}

impl Transport for UsbTransport {
    fn reg_read(&mut self, reg: u32, len: usize) -> Result<Vec<u8>, Error> {
        let (value, index) = Self::reg_value(reg);
    ...
    fn api_command(&mut self, command: &[u8]) -> Result<(), Error> {
        if command.len() != 6 {
            return Err(Error::InvalidArgument("API commands must be six bytes"));
        }
        let mut data = command.to_vec();
        data[1] = self.sequence;
    ...
    fn ack(&mut self, timeout: Duration) -> Result<Vec<u8>, Error> {
        let mut b = [0u8; 64];
        let n = self.handle.read_interrupt(EP_ACK, &mut b, timeout)?;
        Ok(b[..n].to_vec())
    }
    fn firmware(&mut self, data: &[u8]) -> Result<(), Error> {
        if data.len() < 8 || &data[..8] != b"MB8AC018" {
        ...
        for chunk in data.chunks(512) {
            let n = self
                .handle
                .write_bulk(EP_FIRMWARE, chunk, Duration::from_secs(3))?;
            if n != chunk.len() {
                return Err(Error::ShortTransfer {
                    expected: chunk.len(),
                    actual: n,
    ...
    fn i2c_write(&mut self, data: &[u8]) -> Result<(), Error> {
        if data.is_empty() || data.len() > 255 {
            return Err(Error::InvalidArgument("I2C write length out of range"));
        }
    ...
    fn i2c_read(&mut self, register: u8, len: usize) -> Result<Vec<u8>, Error> {
        if len == 0 || len > 255 {
            return Err(Error::InvalidArgument("I2C read length out of range"));
        }
        let mut address = [register];
        self.control(
            0x40,
            I2C,
            0,
        ...
        let mut data = vec![0; len];
        let n = self.control(0xc0, I2C, 0, I2C_INDEX, &mut data, Duration::from_secs(3))?;
        if n != len {
            return Err(Error::ShortTransfer {
                expected: len,
                actual: n,
    ...
    fn ts_read(&mut self, data: &mut [u8], timeout: Duration) -> Result<usize, Error> {
        Ok(self.handle.read_bulk(EP_TS, data, timeout)?)
    }
}
...
pub fn validate_packet_buffer(data: &[u8]) -> bool {
...
mod tests {
    ...
    fn firmware_header_is_checked_without_logging_contents() {
        ...
        assert!(matches!(
            validate_firmware(b"bad"),
            Err(Error::InvalidFirmware)
        ));
    }
    #[test]
    fn register_fields_match_usb_encoding() {
        assert_eq!(UsbTransport::reg_value(0x82008), (0x800, 0x2008));
    }
}
