...
use crate::{device::Transport, Error};
...
pub fn prepare_firmware<T: Transport>(transport: &mut T, firmware: &[u8]) -> Result<u16, Error> {
    ...
    if state == 0 {
        transport.reg_write(REG_IRQ_STATUS, &[0, 4])?;
        transport.firmware(firmware)?;
        transport.reg_write(REG_BOOT, &[0, 4])?;
    }
    Ok(state)
}
...
pub fn wait_state_change<T: Transport>(
    transport: &mut T,
    timeout: Duration,
    tries: usize,
) -> Result<(), Error> {
    for _ in 0..tries {
        let ack = transport.ack(timeout)?;
        if ack.first() == Some(&0x20) {
            return Ok(());
        }
    }
    Err(Error::ProtocolUnavailable(
        "expected state-change ACK was not received",
    ))
}
...
const TUNER_INIT: &[&[u8]] = &[
    ...
    &[0xef, 1],
    &[
        0xfe, 0xc0, 0, 0x3f, 2, 0, 3, 0x48, 4, 0, 5, 4, 6, 0x10, 0x2e, 0x15, 0x30, 0x10, 0x45,
        0x58, 0x48, 0x19, 0x52, 3, 0x53, 0x44, 0x6a, 0x4b, 0x76, 0, 0x78, 0x18, 0x7a, 0x17, 0x85,
        6,
    ],
    &[0xfe, 0xc0, 1, 1],
];

pub fn init_tuner<T: Transport>(transport: &mut T) -> Result<(), Error> {
    for entry in TUNER_INIT {
        transport.i2c_write(entry)?;
    }
    Ok(())
}

pub fn tune_channel<T: Transport>(transport: &mut T, channel: u8) -> Result<(), Error> {
    if !(13..=62).contains(&channel) {
        return Err(Error::InvalidArgument("channel must be between 13 and 62"));
    }
...
pub fn activate_trc<T: Transport>(transport: &mut T, firmware: &[u8]) -> Result<(), Error> {
    for reg in (0x1000..0x1500).step_by(2) {
        transport.reg_write(reg, &[0, 0])?;
    }
    for &(reg, hi, lo) in &[
        (0x1002, 0x84, 4),
        (0x1004, 1, 0x84),
        ...
        transport.reg_write(reg, &[hi, lo])?;
    }
    transport.api_command(&[0, 0, 4, 0, 0, 0])?;
    require_ack(transport, Duration::from_secs(1))?;
    transport.firmware(firmware)?;
    transport.api_command(&[0, 0, 4, 0x20, 0, 0])?;
    require_ack(transport, Duration::from_secs(1))?;
    wait_state_change(transport, Duration::from_millis(500), 20)
}

pub fn start_stream<T: Transport>(transport: &mut T) -> Result<(), Error> {
    transport.api_command(&[0, 0, 5, 0, 0, 2])?;
    require_ack(transport, Duration::from_secs(1))
}
fn require_ack<T: Transport>(transport: &mut T, timeout: Duration) -> Result<(), Error> {
    if transport.ack(timeout)?.is_empty() {
        Err(Error::ProtocolUnavailable("empty device ACK"))
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    fn channel_frequency_is_bounded_and_encoded() {
        assert_eq!(pll_divider(13).unwrap(), 0x7649);
